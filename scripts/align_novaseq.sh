...
out_dir=aligned_data/STAR-aligned_UMI-extracted/$sample_ID/
mkdir -p $out_dir
...
if [ ! -f $out_dir/${sample_ID}_Aligned.out.bam ]; then
    STAR --runThreadN $ncpu --quantMode GeneCounts --genomeDir $GENOME_DIR \
         ...
         --readFilesIn $out_dir/umi-extracted_R1.fastq.gz $out_dir/umi-extracted_R2.fastq.gz \
...
if [ ! -f $out_dir/${sample_ID}_counts.tsv.gz_ ]; then
	umi_tools count --per-gene --gene-tag=XT --assigned-status-tag=XS --per-cell \
			--wide-format-cell-counts \
			-I $out_dir/${sample_ID}_assigned_sorted.bam -S $out_dir/${sample_ID}_counts.tsv.gz| tee $out_dir/umi_tools_count.log
fi
