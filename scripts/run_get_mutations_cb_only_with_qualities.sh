...
#PBS -m ae
#PBS -o logs/mutations-cb-only-with-qualities/
#PBS -j oe
#PBS -J 1-4


mkdir -p logs/mutations-v2/
...
outdir="results/mutations-called-cb-only-with-qualities_v2/"
mkdir -p $outdir

output="$outdir/mutations-called_${sample}.csv"
python3.10 scripts/get_mutations_cb_only_with_qualities.py -s $sam -m $mutations -o $output -c $NCPU
