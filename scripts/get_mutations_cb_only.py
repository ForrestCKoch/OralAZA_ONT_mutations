...
def get_mutation_dict(fetch, header, mt_df, quiet=False, cores=8):
    ...
    for cb, mt_status in results:
        ...
        for mutation in mt_status:
            ...
            mt_dict[cb][mutation][mt_status[mutation]] += 1
...
def get_cb_mutations(mt_dict, quiet=False):
    ...
    for cb in tqdm(mt_dict):
        ...
        for mutation, mts in mt_dict[cb].items():
            ...
            n_wt = mt_dict[cb][mutation]['WT'] if 'WT' in mt_dict[cb][mutation] else 0
            n_mt = mt_dict[cb][mutation]['MT'] if 'MT' in mt_dict[cb][mutation] else 0
            n_un = mt_dict[cb][mutation]['UN'] if 'UN' in mt_dict[cb][mutation] else 0

            cb_mutations[cb][mutation] = (n_wt, n_mt, n_un)
